...
DeprecatedFunctions = function (sol_file: SolFile, plugin_config: PluginConfig): IssuePointer[] {
  ...
  for (const c of sol_file.contracts_current) {
    const identifiers: Identifier[] = SolidityAntlr.parseIdentifier(c.subNodes.branch);
    for (const i of identifiers) {
      if (AstUtility.matchRegex(i.name, new RegExp("sha3")) || AstUtility.matchRegex(i.name, new RegExp("suicide"))) {
        issuePointers.push(new IssuePointer(plugin_config.swcID, i.location));
      }
    }
  }
